...
let rec subst_type a b = match b with
                        TyVar c ->
                          (match a with
                              [] -> b
                            | (d, e) :: rest -> if d = c then subst_type rest e else subst_type rest b )
...
let rec ty_exp tyenv = function
  ...
  | LetRecExp (id, para, exp1, exp2) -> let dummyTy = TyVar( fresh_tyvar() ) in
                                        let paraty1 = TyVar( fresh_tyvar() ) in
                                        let dummyEnv = Environment.extend para (TyScheme([], paraty1)) (Environment.extend id (TyScheme([], dummyTy)) tyenv) in
                                        let (s1, ty2) = ty_exp dummyEnv exp1 in
                                        let eqs = [(dummyTy, TyFun(paraty1, ty2))] in
                                        let subst1 = unify ((eqs_of_subst s1) @ eqs) in
                                        let ty3 = subst_type subst1 dummyTy in
                                        let tysc = closure ty3 tyenv s1 in
                                        let (s3, ty5) = ty_exp (Environment.extend id tysc tyenv) exp2 in
                                        let subst2 = unify(eqs_of_subst (subst1 @ s3)) in  (subst2, subst_type subst2 ty5)
...
let ty_decl tyenv = function
  ...
  | RecDecl (id, para, exp) ->  let dummyTy = TyVar( fresh_tyvar() ) in
                                let paraty1 = TyVar( fresh_tyvar() ) in
                                let newenv1 = Environment.extend para (TyScheme([], paraty1)) (Environment.extend id (TyScheme([], dummyTy)) tyenv) in
                                let (s1, ty2) = ty_exp newenv1 exp in
                                let eqs = [(dummyTy, TyFun(paraty1, ty2))] in
                                let subst1 = unify (eqs @ (eqs_of_subst s1)) in
                                let ty3 = subst_type subst1 dummyTy in
                                let tysc = closure ty3 tyenv s1 in (ty3, Environment.extend id tysc tyenv)
